...
def test_runtime_ssh_readiness_poll_error_is_fixed_without_config_cleanup(
    tmp_path: Path,
    ...
    config = RuntimeConfig.model_validate(
        {
            "system": {
                "ssh": {
                    "enable": True,
                    "password": "lifecycle-ssh-password-sentinel",
                }
            }
        }
    )
    ...
    class PollErrorSshd(FakeSshdProcess):
        def poll(self) -> int | None:
            raise OSError("raw-lifecycle-poll-sentinel")

    process = PollErrorSshd()
    ...
    def starter(
        ...
        *,
        ...
        return start_sshd_if_enabled(
            ...
            runtime_dir=tmp_path / "run" / "sshd",
            config_dir=config_dir,
            credential_command_runner=RecordingSensitiveCommandRunner(),
            config_owner_uid=os.getuid(),
            config_owner_gid=os.getgid(),
            command_runner=RecordingCommandRunner(),
    ...
    service = RuntimeSshService(
        config,
        environment={b"POLL_SECRET": b"lifecycle-private-value"},
        starter=starter,
        background_event_sink=RecordingRuntimeEventSink(),
        event_sink=RecordingRuntimeEventSink(),
    )

    with pytest.raises(RuntimeSshServiceError) as raised:
        service.start()

    captured = capsys.readouterr()
    payload = f"{raised.value}\n{captured.out}\n{captured.err}"
    assert str(raised.value) == "SSH startup/readiness failed"
    assert "raw-lifecycle-poll-sentinel" not in payload
    assert "lifecycle-ssh-password-sentinel" not in payload
