...
原子重命名是替换操作的提交点。之后的耐久性、验证、清理或恢复状态持久化失败仍会使该操作失败，但完整的新文件可能已经保留在目标位置；cdh 不会将其回滚为旧文件。
...
使用保留的状态和下载目标启动替代容器前，必须先停止旧的 cdh 容器。不要让重叠运行的滚动更新实例或多个副本共享可写状态，也不要共享由该状态管理的目标和暂存文件。即使使用不同的状态文件，只要并发写入者控制的是同一个下载目标或暂存命名空间，也不属于受支持的用法。
