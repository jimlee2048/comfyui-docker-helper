...
The renderer and final materializer obtain the package-provided static SSH-conditional profile artifact from the exact retained canonical wheel; runtime does not create, modify, or clean profile state. The project-provided root Bash login shell consumes the artifact when associated with SSH, so both an interactive login and an explicitly requested `bash -lc` enter effective `WORKSPACE`, with a fixed `/root` fallback warning when that directory cannot be entered. An ordinary remote command does not read the profile, retains native OpenSSH execution and `/root` cwd, and is not mediated by ForceCommand, PAM synchronization, or a downstream-shell compatibility layer.
