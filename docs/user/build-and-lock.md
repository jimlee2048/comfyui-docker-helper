...
Every target-active named direct Python reference is a moving request even when its URL text contains a version, hash fragment, or VCS ref. Default reconciliation and `--check` reuse a compatible existing exact version when the complete request digest, including the authored source string, is unchanged; they acquire it when the entry is missing or incompatible. `--locked` accepts only that existing matching request/result without contacting the source during host-side reconciliation. `--upgrade-lock` reacquires every moving direct reference. These policies govern when cdh asks uv to resolve the source; they do not turn a URL or VCS ref into an artifact lock. A subsequent Buildx build still fetches and installs each target-active authored direct source.
...
For each active package request, the canonical request digest includes the normalized package identity and any authored direct source. The canonical lock preserves the matching request and exact resolved top-level version but does not lock the source artifact. The BuildPlan restores the target-active authored source from the accepted request and combines it with the locked expected version, so execution uses the requested source and final evidence still requires exact version equality.

Package direct references are ordinary public configuration, not Secret locators. Every target-active literal enters the BuildPlan and rendered context, so it may be visible to the builder, build cache, and anyone who can read that context. A target-active configured direct uv-tool reference is additionally rendered into a Dockerfile `RUN` instruction and may appear in image history; active application and PyTorch direct references are read from the mounted BuildPlan rather than copied into Dockerfile instruction text. URL userinfo is rejected, and cdh does not attach downloader/Git credential routes to package installation. Never put tokens or private credentials in these references.
