...
| S4 | Manager on, no nodes | `comfyui.version="0.11.0"`, `install_cli=false`, `install_manager=true`, no custom nodes | S4 command | exact checkout declaration, `comfyui-manager==4.0.5`, read-only import anchor, absolute `/opt/venv/bin/cm-cli`, application dependency health, and unchanged runtime arguments | Manager is available from `/opt/venv` without comfy-cli; its import and cm-cli bind to the exact ComfyUI workspace, and cdh does not append `--enable-manager`. |
| S5 | registry node | formal `comfyui.version="0.11.0"`, `install_cli=false`, Manager 4.0.5, exact `comfyui-custom-scripts@1.2.5` | S5 command | one absolute public cm-cli process, exact flags/closed input, safe root metadata proof, schema-1 Registry inventory, application dependency health | Clean-cache exact install succeeds without comfy-cli; the root project identity/version and declaration-ordered inventory are exact, and false-zero dispositions fail before a later node. |
