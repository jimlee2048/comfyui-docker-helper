...
| [Materialization](../../src/comfyui_docker_helper/rendering/final_materializer.py) | Private-stage projection of one BuildPlan plus exact release/local inputs, without planning or resolution. The host service owns context publication and comparison. See [materialization](contracts.md#materialization-boundary). |
