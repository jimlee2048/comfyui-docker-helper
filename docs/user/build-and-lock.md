...
Directory application is an overlay, not a mirror. Selected files and directories replace compatible entries below the target while unrelated lower-image entries remain; a file/directory conflict or an incompatible target root fails. Every selected directory is normalized to `0755` and every selected regular file to `0644`; host ownership, timestamps, ACLs, xattrs, and executable bits are not image authority. All effective target regions must be disjoint by equality and component-prefix overlap. Target and local-tree member names must follow the [file naming rules](configuration.md#add-files-during-the-image-build), which reserve `.cdh-staging` and the `.wh.` prefix. A completely empty local directory is valid and still materializes its target root.
...
With `content_lock = false`, ordinary planning does not hash local source bytes. A local file has no content digest; a local tree's BuildPlan still freezes its complete sorted inventory of relative members and node kinds, with output modes fixed by kind. `--locked` compares that tree structure without comparing unlocked bytes, while `--check` compares complete membership and streams every source/context file for byte equality. With `content_lock = true`, a local file gets one target-keyed SHA-256 digest in the BuildPlan and one matching canonical-lock row; a local tree records each regular member's size and SHA-256 plus one aggregate tree identity in the BuildPlan, while the canonical lock stores one aggregate `sha256:` entry rather than duplicating its member inventory. Materialization and final observation revalidate the accepted inventory and identity. These operations are bounded-memory but necessarily read complete files when their result requires it.

Unlocked admission safely opens local files without reading their complete contents, including during `--dry-run`. Read errors encountered later during copy or explicit `--check` fail that operation; an incomplete materialization is not published. Successful unlocked clone also does not prove that every data block has been read, so Docker consumption can still surface a read error. Source safety and structural checks remain in place; see the [host filesystem contract](../dev/contracts.md#cooperative-source-reads).

For locked input, ordinary copy hashes the bytes being transferred; successful clone hashes the acquired private copy instead of rereading the source. This verifies the input prepared for Docker against the Plan, without an additional full-content read after ordinary copy. Source structural checks remain, but do not rehash content after acquisition; see the [materialization contract](../dev/contracts.md#materialization-boundary).
...
After all image mutations succeed, cdh writes the strict final-state observation `/opt/cdh/build/manifest.json`. It binds the image-configuration, canonical lock, and BuildPlan digests and records verified final-state evidence. A local tree contributes one compact row containing its kind, target, verification mode, and—when content-locked—the observed aggregate tree digest after comparison with Plan intent; the row does not duplicate members, counts, or host paths. The final observer checks only Plan-selected roots and members, so unrelated lower-image entries remain outside its evidence. The manifest is evidence, not another resolver, lock, replay input, support verdict, or general service-health check.
