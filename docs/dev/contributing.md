...
Typer owns help, usage, option parsing, and parameter-error presentation. External process output such as BuildKit, uv, Git, or ComfyUI logs remains an unparsed, unstyled, unprefixed stream on its established channel; cdh may frame that stream but does not rewrite it. Consequently, cdh's ANSI-free and `NO_COLOR` guarantees do not extend to controls emitted by an external process itself. Image-internal `cdh container` commands retain their simple plain execution and logging protocols rather than using the host operator presenter.
