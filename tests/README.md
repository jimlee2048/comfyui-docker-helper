...
Runtime controller state, framed protocol validation, and log-fanout policy belong in unit or focused integration tests at their owning boundary. Real Unix-domain sockets, inherited or redirected process descriptors, subprocess clients, and controlling-terminal interruption belong in integration tests; these local operating-system boundaries do not require Docker or network authorization.

Give parameterized cases concise, stable IDs when their values are large, binary, control-bearing, or otherwise unsuitable for display. Parameterize large boundaries with a concise descriptor or size and construct the exact payload in the test body so collection does not retain it as case identity. Pytest exposes node IDs through process state and CI logs, so retain the full payload as test data without allowing it to become the generated case name. Keep failure evidence for extreme or sensitive values content-safe by reporting stable labels, lengths, digests, or fixed safe samples instead of raw payloads. This keeps platform limits and diagnostic output independent from the boundary value being tested. The shared collection policy rejects a complete node ID longer than 4,096 characters before test execution and reports only the count and observed lengths, never the node ID content.
...
Lifecycle changes reuse one current image produced by the formal renderer; do not create a second handwritten Dockerfile for lifecycle tests. Set `CDH_LIFECYCLE_CONTEXT` to that rendered context and `CDH_LIFECYCLE_IMAGE` to the image built from it, then run `tests/smoke/test_lifecycle_shutdown_live.py` with `--run-docker --run-slow`. The suite bind-mounts deterministic programs, runtime config, and hooks into that image to cover stable Tini/cdh topology across complete generation restart, retained history and `logs --follow` across restart with intact Docker streams, same-container restart persistence and the configured raw log directory, accepted-client interruption, restart-failure cleanup and exit, first/repeated signals, finite and disabled cdh deadlines, active-hook cancellation, paired background-service shutdown, natural exit, and adopted-orphan reap. Separate real process-control integration proves cdh-managed child reap inside the ownership window; the formal force test also holds cdh during interpreter exit and observes that its direct application child is already absent from `/proc`. Container exit alone is not that evidence. Graceful service shutdown and Tini orphan-zombie reap also require their own observed markers. The release image matrix repeats only lightweight PID topology, default SIGTERM, and ordinary application shutdown checks.
