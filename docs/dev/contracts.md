...
- Host Docker transport is split narrowly: `python-on-whales` owns exact uv image pull and inspection, resolver-container lifecycle, and Buildx; the official Docker SDK owns only Docker Engine distribution metadata acquisition. Neither transport dependency is serialized as an image identity.
