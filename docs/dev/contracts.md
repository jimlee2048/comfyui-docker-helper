...
Registry Manager and the Direct-Git Python installation processes for root requirements and `install.py` receive the BuildPlan-owned ordinary and CUDA-derived PyTorch indexes plus two constraints projected from the same typed PyTorch group. Runtime constraints retain every exact group result and the selected torch wheel's setuptools compatibility, while the isolated-build projection retains the exact group results and does not itself add that application-only setuptools range. An installer's version-specific handling of ordinary runtime constraints inside build isolation remains installer-owned. uv considers compatible candidates across both owned indexes instead of allowing the CUDA index's ordinary package names to hide the ordinary index, matching the shared pip path's merged-candidate behavior. Ambient pip and uv configuration cannot replace these inputs. Supplying them controls the package sources and protected versions available to trusted installers; it does not make dependencies selected inside those installers canonical-lock or BuildPlan identities.
