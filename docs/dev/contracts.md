...
The renderer and final materializer own the package-provided static SSH-conditional profile artifact as image input; runtime does not create, modify, or clean profile state. The artifact is consumed only by the project-provided root Bash interactive SSH login to enter effective `WORKSPACE`, with a fixed `/root` fallback warning when that directory cannot be entered. It does not mediate remote commands, which retain native OpenSSH execution and `/root` cwd, and it is not a ForceCommand, PAM synchronization, or downstream-shell compatibility contract.
